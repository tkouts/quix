...
var checkNativeSupport = function () { return false; };
...
function toCamelCase (prop) {
  return prop.replace(camelRe, function (all, char) { return ("" + char).toUpperCase(); })
}
...
function borderIncludedInOffset () {
  ...
  var isIncluded = (div.firstChild.offsetTop === 1);
...
if (typeof window.CustomEvent !== 'function') {
  ...
  CustomEvent$1.prototype = Event.prototype;
...
if (!capabilities.cssPointerEvents) {
  ...
  var passThrough = function passThrough (e) {
    var target = e.target;
    var disabled = null;
    ...
    if (target) {
      ...
      if (disabled) {
        ...
        var origDisplay = disabled.$el.style.visibility;
        disabled.$el.style.visibility = 'hidden';

        var underneathElem = document.elementFromPoint(e.clientX, e.clientY);

        // fire the mouse event on the element below
        var evt = new CustomEvent(e.type, {
          cancelable: true,
          bubbles: true
        });
        // Object.assign(evt, e);
...
function onPointerMove (evt) {
  ...
  var value = el.$qx_movable.value;
  var arg = el.$qx_movable.arg;
  // console.log(binding);
  ...
  var cancelled = !el.dispatchEvent(cr);
  if (!cancelled) {
    if (newX) {
...
function onPointerDown (evt) {
  var offsetLeft = this.__vue__.getInnerLeft();
  var offsetTop = this.__vue__.getInnerTop();
  if (!this.__vue__.abs) {
    // calculate existing offsets
    var parent = this.__vue__.$parent;
...
__$styleInject(".qxw{display:block;border:0 solid #000;margin:0;padding:0;overflow:hidden;box-sizing:border-box;position:relative;line-height:normal}.qxw.abs{position:absolute}.qxw.fixed{position:fixed}.qxw.no-clip{overflow:visible}.qxw.disabled{pointer-events:none;opacity:.5;cursor:default}.qxw.disabled *{cursor:default!important}.qxw.text-align-center{text-align:center}.qxw.text-align-start{text-align:left}.qxw.text-align-end{text-align:right}div.valign-container{white-space:nowrap;height:100%}div.valign-container:before{content:'';display:inline-block;height:100%;width:0;vertical-align:middle}div.valign-container>.valign-start{display:inline-block;vertical-align:top}div.valign-container>.valign-center{display:inline-block;vertical-align:middle}div.valign-container>.valign-end{display:inline-block;vertical-align:bottom}",undefined);

// common prop types

function mutableInt (dflt) {
...
function mutableString (dflt, allowedValues) {
  if ( dflt === void 0 ) dflt = '';
  ...
  if (allowedValues.length) {
    propDef.validator = function (val) { return allowedValues.indexOf(val) > -1; };
  }
  return propDef
}
...
function mutablePatcher (options) {
  var $options = options;
  $options.mProps = {};
  // patch $options with computed properties for mutable props
  Object.keys($options.props).forEach(function (propName) {
    var propDef = $options.props[propName];
    if (propDef.mutable) {
      var computedName = "$" + propName;
...
var repaintsInProgress = 0;
var repaintStack = [];
var repaintMap = {};

function updateGeometry (comp, computes) {
  var component = comp;
  ...
  // inner-width
  if (computes.iw) {
    var offset = component.paddingLeft + component.paddingRight +
  ...
  // inner-height
  if (computes.ih) {
    var offset$1 = component.paddingTop + component.paddingBottom +
      component.borderTop + component.borderBottom;
    component.rect.ih = component.rect.oh - offset$1;
  ...
  if (computes.sw || computes.sh) {
    var shouldContainX = component.$width === 'contain' ||
      component.$minWidth === 'contain';
    var shouldContainY = component.$height === 'contain' ||
...
function shouldUpdate (comp) {
  var computes = comp.app.dynamic.components[comp._uid].computes;
  var update = false;
...
function repaint () {
  ...
  var dynamic = this.app.dynamic.components;
  var dynamicKeys = Object.keys(dynamic);
  var totalDynamic = dynamicKeys.length;
  var forUpdate = [];

  // get contained componens
  for (var i = 0; i < totalDynamic; i += 1) {
    var uid = dynamicKeys[i];
    var comp = dynamic[uid].c;
    if (this$1 === this$1.app || this$1.repaintBox.$el.contains(comp.$el)) {
      forUpdate.push(comp);
    }
  }
  ...
  var updateLength = forUpdate.length;
  for (var i$1 = 0; i$1 < updateLength; i$1 += 1) {
    var comp$1 = forUpdate[i$1];
    if (shouldUpdate(comp$1)) {
      updateGeometry(comp$1, dynamic[comp$1._uid].computes);
    }
  }
  vue.nextTick(finishRepaint);
}

function componentUpdated$1 () {
  var this$1 = this;

  if (this.app.ready) {
    var dynamicComponents = this.app.dynamic.components;
    var dynamic = dynamicComponents[this._uid];
    if (dynamic) {
    ...
    if (!(repaintStack.some(function (c) { return c.$el.contains(this$1.repaintBox.$el); }))) {
      this.repaintBox.$nextTick(repaint);
    }
  }
}

var reactiveFunctionCounter = 0;
var reactiveStack = [];

var PartialComponents = function PartialComponents (reactiveFunctionId) {
  this.components = {};
  this.rId = reactiveFunctionId;
};

PartialComponents.prototype.add = function add (component, compute) {
  var componentId = component._uid;
  var entry = this.components[componentId];
  if (!entry) {
    entry = {
      c: component,
      computes: {},
...
var MasterComponents = function MasterComponents () {
  this.partials = {};
  this.components = {};
};

MasterComponents.prototype.add = function add (partial) {
    var this$1 = this;
  ...
  var componentKeys = Object.keys(partial.components);
  for (var i = 0; i < componentKeys.length; i += 1) {
    var uid = componentKeys[i];
    var masterEntry = this$1.components[uid];
    var entry = partial.components[uid];
    // console.log('adding', entry)
    if (!masterEntry) {
      this$1.components[uid] = Object.assign({}, entry);
      this$1.components[uid].computes = Object.assign({}, entry.computes);
    } else {
      var computesKeys = Object.keys(entry.computes);
      for (var j = 0; j < computesKeys.length; j += 1) {
        var compute = computesKeys[j];
...
MasterComponents.prototype.remove = function remove (partial) {
    var this$1 = this;
...
MasterComponents.prototype.removeComponent = function removeComponent (component) {
    var this$1 = this;

  var uid = component._uid;
  if (this.components[uid]) {
    delete this.components[uid];
    // remove partials
    var partialsKeys = Object.keys(this.partials);
    for (var i = 0; i < partialsKeys.length; i += 1) {
      var partialKey = partialsKeys[i];
      if (partialKey.startsWith((uid + "/"))) {
        this$1.remove(this$1.partials[partialKey]);
        delete this$1.partials[partialKey];
      }
    }
  }
};

function reactive (func, def) {
  return function reactiveWrapper () {
    if (!this.ready) { return def }
    // var value, partial;
    var reactiveFunc = func;
    if (!reactiveFunc.rId) {
      reactiveFunctionCounter += 1;
      reactiveFunc.rId = reactiveFunctionCounter;
    }
    var partial = new PartialComponents(((this._uid) + "/" + (reactiveFunc.rId)));
    reactiveStack.push(partial);
    var value = reactiveFunc.call(this);
...
function geometryWatcher (compute, force) {
...
function calc (prop, value) {
  ...
  if (!isNaN(computed) && computed != null) {
    ...
    var axis = getAxis(prop);
    var root = this.parent.$refs.root;
    var parent = this.parent;
    if (root) {
      var rootEl = root.$el || root;
    ...
    var func = new Function(("return " + (computed.slice(3))));
...
Object.assign(funCache, {
  ...
  'flex-compute': function flexCompute (prop) {
    ...
    var parent = this.parent;
    var size = parent.floatingSpace * (this.$flex / parent.flexCount);
    if (parent.flow) {
...
function getCssBoxMetric (val) {
  if (val) {
    return val.map(function (v) { return (isNaN(v) ? v : (v + "px")); }).join(' ')
  }
  return null
}
...
var height = Object.assign({}, dynamicAttribute, {
  get: function get (value) {
    if (!capabilities.flexSupported && this.$parent.orientation) {
      var orient = this.$parent.orientation;
...
var width = Object.assign({}, dynamicAttribute, {
...
var bottom = Object.assign({}, dynamicAttribute, {
  get: function get$2 (value) {
    if (!capabilities.flexSupported && this.$parent.orientation === 'h' &&
      ...
      var heightAvailable = this.$parent.innerHeight +
...
var rect = {
render: function(){with(this){return _h('div',{staticClass:"qxw",class:classes,style:([boxStyle, paddingStyle, sizeStyle, positionStyle])},[_t("default")])}},
staticRenderFns: [],
  name: 'qx-rect',
  class: {},
  props: {
  ...
  mounted: function mounted () {
    ...
    if (root) {
      if (root.$el) {
        ...
        var parentElement = root.parentNode;
  ...
  computed: {
    ...
    computedFlexAlign: function computedFlexAlign () {
      if (!capabilities.flexSupported && this.$parent.orientation) {
    ...
    overlays: function overlays () {
      return this.children.filter(function (c) { return c.showOn; })
    },
    // Geometry
    ...
    hasVariableWidth: function hasVariableWidth () {
      if (this.parent === this.$root && this.$width == null) {
        return true
      }
    ...
    hasVariableHeight: function hasVariableHeight () {
      if (this.parent === this.$root && this.$height == null) {
        return true
      }
    ...
    shouldUpdateParent: function shouldUpdateParent () {
    ...
    repaintBox: function repaintBox () {
      var box = this.$parent;
      // if (this.hasVariableWidth || this.hasVariableHeight) {
      //  box = this.$parent
  ...
  methods: {
    contains: function contains (el) {
      ...
      if (this.overlays.length > 0) {
        contains = this.overlays.some(function (o) { return o.contains(el); });
      }
      contains = contains || this.$el.contains(el);
      return contains
    },
    // DOM manipulation
    before: function before (Vue, propsData) {
...
__$styleInject(".qxw.app{width:100%;height:100%;font-family:Arial,Helvetica,sans-serif;font-size:14px}",undefined);
...
var overlayContainer = {
  ...
  methods: {
    closeOverlay: function closeOverlay (evt) {
  ...
  watch: {
    activeOverlay: function activeOverlay (newOverlay, oldOverlay) {
      if (oldOverlay) {
        var old = oldOverlay;
        old.open = false;
      }
...
var app = {
render: function(){with(this){return _h('div',{staticClass:"qxw app",class:classes,style:([boxStyle, paddingStyle, sizeStyle, positionStyle]),attrs:{"touch-action":touchAction},on:{"!pointerdown":function($event){closeOverlay($event)}}},[_t("default")])}},
staticRenderFns: [],
  name: 'qx-app',
...
__$styleInject(".qxw.scroller>div{position:relative;height:100%}.qxw.scroller.desktop .iScrollHorizontalScrollbar{box-sizing:border-box;position:absolute;height:12px;left:0;right:0;top:100%}.qxw.scroller.desktop .iScrollVerticalScrollbar{box-sizing:border-box;position:absolute;width:12px;bottom:0;top:0;left:100%}.qxw.scroller.desktop .iScrollIndicator{box-sizing:border-box;position:absolute;background:#8d8d8b;border:1px solid #666}.qxw.scroller.desktop .iScrollHorizontalScrollbar .iScrollIndicator{height:100%}.qxw.scroller.desktop .iScrollVerticalScrollbar .iScrollIndicator{width:100%}",undefined);
...
function refreshScroller () {
  ...
  if (!this.refreshTimeout) {
    this.refreshTimeout = setTimeout(function () {
      this$1.scroller.refresh();
      this$1.refreshTimeout = null;
    }, 60);
  }
}

var scroller = {
render: function(){with(this){return _h('div',{staticClass:"qxw scroller",class:classes,style:([boxStyle, paddingStyle, sizeStyle, positionStyle])},[_h('div',[_h('qx-rect',{ref:"root",staticClass:"scroller-container",attrs:{"height":"100%","min-width":containerWidth,"min-height":containerHeight,"padding":$padding}},[_t("default")])])])}},
staticRenderFns: [],
  name: 'qx-scroller',
  mixins: [rect],
  props: {
    ...
    probe: {
      type: Number,
      default: 0,
      validator: function (val) { return [0, 1, 2].indexOf(val) > -1; }
    }
  },
  data: function data () {
    return {
  ...
  mounted: function mounted () {
    ...
    var options = Object.assign({}, iScrollOptions, {
      scrollX: this.$scrollX,
      scrollY: this.$scrollY,
      probeType: this.probe
    });
    this.scroller = new iscrollProbe(this.$el.firstChild, options);
    ...
    this.scroller.on('scrollEnd', function () {
      ...
      this$1.$emit('scrollend');
    });
    if (this.probe) {
      this.scroller.on('scroll', function () {
        this$1.scrollTop = this$1.y;
        this$1.scrollLeft = this$1.x;
        this$1.$emit('scroll');
      });
    }
  },
  ...
  computed: {
    classes: function classes () {
      var classes = rect.computed.classes.call(this);
...
__$styleInject(".qxw.label{cursor:default;white-space:nowrap;text-align:start;text-overflow:ellipsis}.qxw.label.wrap{white-space:normal}",undefined);

var label = {
render: function(){with(this){return _h('label',{staticClass:"qxw label",class:classes,style:([boxStyle, paddingStyle, sizeStyle, positionStyle])},["\n  "+_s($text)+"\n"])}},
staticRenderFns: [],
  name: 'qx-label',
  mixins: [rect],
  props: {
...
var image = {
render: function(){with(this){return _h('img',{staticClass:"qxw image",class:classes,style:([boxStyle, paddingStyle, sizeStyle, positionStyle]),attrs:{"src":src}})}},
staticRenderFns: [],
  name: 'qx-image',
  mixins: [rect],
  props: {
...
var iframe = {
render: function(){with(this){return _h('iframe',{staticClass:"qxw image",class:classes,style:([boxStyle, paddingStyle, sizeStyle, positionStyle]),attrs:{"src":src}})}},
staticRenderFns: [],
  name: 'qx-image',
  mixins: [rect],
  props: {
...
staticRenderFns: [],
  ...
  props: {
    ...
    value: {
      ...
      mutable: true,
      set: function set (val) {
        var value = Math.round(parseFloat(val) * Math.pow(10, this.decimals)) /
  ...
  computed: {
    handleOffset: function handleOffset () {
      var x = (this.$value - this.min) / (this.max - this.min);
      return ((x * 100) + "%")
    }
  },
  methods: {
    update: function update (evt) {
      var oldValue = this.$value;
      var x = evt.detail.x;
      var value = ((this.max - this.min) * (x / this.$refs.slot.getOuterWidth())) + this.min;
      this.$value = value;
...
__$styleInject(".qxw.box{display:-webkit-box;display:-ms-flexbox;display:flex}.qxw.box>*{-webkit-box-flex:0;-ms-flex:none;flex:none}.qxw.box.flow{-ms-flex-wrap:wrap;flex-wrap:wrap;-ms-flex-line-pack:start;align-content:flex-start}.qxw.box.align-start{-webkit-box-align:start;-ms-flex-align:start;align-items:flex-start}.qxw.box.align-center{-webkit-box-align:center;-ms-flex-align:center;align-items:center}.qxw.box.align-end{-webkit-box-align:end;-ms-flex-align:end;align-items:flex-end}.qxw.box.align-stretch{-webkit-box-align:stretch;-ms-flex-align:stretch;align-items:stretch}.qxw.box.justify-start{-webkit-box-pack:start;-ms-flex-pack:start;justify-content:flex-start}.qxw.box.justify-center{-webkit-box-pack:center;-ms-flex-pack:center;justify-content:center}.qxw.box.justify-end{-webkit-box-pack:end;-ms-flex-pack:end;justify-content:flex-end}.qxw.box>.qxw.self-align-start{-ms-flex-item-align:start;align-self:flex-start}.qxw.box>.qxw.self-align-end{-ms-flex-item-align:end;align-self:flex-end}.qxw.box>.qxw.self-align-center{-ms-flex-item-align:center;-ms-grid-row-align:center;align-self:center}.qxw.box>.qxw.self-align-stretch{-ms-flex-item-align:stretch;-ms-grid-row-align:stretch;align-self:stretch}",undefined);
...
var hbox$1 = {
render: function(){with(this){return _h('div',{staticClass:"qxw box",class:classes,style:([boxStyle, paddingStyle, sizeStyle, positionStyle])},[_t("default")])}},
staticRenderFns: [],
  name: 'qx-hbox',
  mixins: [rect, boxBase],
  mounted: function mounted () {
  ...
  computed: {
    classes: function classes () {
      var cssClass = rect.computed.classes.call(this);
      if (this.$itemsAlign) {
  ...
  methods: {
    applyChildMargins: function applyChildMargins () {
      var this$1 = this;
      ...
      if (this.children.length > 0) {
        if (!this.flow) {
          ...
          for (var i = 1; i < this.children.length; i += 1) {
            this$1.children[i].$margin = [0, 0, 0, this$1.$spacing];
          }
        } else {
          for (var i$1 = 0; i$1 < this.children.length; i$1 += 1) {
            this$1.children[i$1].$margin = [0, this$1.$spacing, this$1.$spacing, 0];
          }
        }
      }
    }
...
__$styleInject(".qxw.hbox-legacy>div{white-space:nowrap;text-align:start}.qxw.hbox-legacy.flow>div{white-space:normal}.qxw.hbox-legacy.justify-start>div{text-align:left}.qxw.hbox-legacy.justify-center>div{text-align:center}.qxw.hbox-legacy.justify-end>div{text-align:right}.qxw.hbox-legacy>div>*,.qxw.hbox-legacy>div>.qxw.self-align-start{vertical-align:top;display:inline-block}.qxw.hbox-legacy.align-center>div>*,.qxw.hbox-legacy>div>.qxw.self-align-center{vertical-align:middle;display:inline-block}.qxw.hbox-legacy.align-end>div>*,.qxw.hbox-legacy>div>.qxw.self-align-end{vertical-align:bottom;display:inline-block}",undefined);

var legacyBoxBase = {
  mounted: function mounted () {
    // remove text nodes
    var rootEl = this.$el.firstChild;
  ...
  computed: {
    flexCount: function flexCount () {
      ...
      for (var i = 0; i < this.children.length; i += 1) {
        if (this$1.children[i].$flex) {
          // TODO: exclude non displayed
          flexSum += this$1.children[i].$flex;
        }
      }
      return flexSum
    }
  ...
  methods: {
    floatingSpace: function floatingSpace () {
...
var hBoxLegacy = {
render: function(){with(this){return _h('div',{staticClass:"qxw hbox-legacy",class:classes,style:([boxStyle, paddingStyle, sizeStyle, positionStyle])},[_h('div',{class:$height && !flow ? 'valign-container' : ''},[_t("default")])])}},
staticRenderFns: [],
  mixins: [hbox$1, legacyBoxBase],
  computed: {
    ...
    floatingSpace: reactive(function floatingSpace () {
      ...
      if (this.flexCount) {
        ...
        for (var i = 0; i < this.children.length; i += 1) {
          // TODO: exclude non displayed
          if (!this$1.children[i].$flex) {
            fixedSpace += this$1.children[i].outerWidth;
          }
          if (i > 0) {
            fixedSpace += this$1.$spacing;
          }
        }
...
__$styleInject(".qxw.box.vertical{-webkit-box-orient:vertical;-webkit-box-direction:normal;-ms-flex-direction:column;flex-direction:column}",undefined);

var vbox$1 = {
render: function(){with(this){return _h('div',{staticClass:"qxw box vertical",class:classes,style:([boxStyle, paddingStyle, sizeStyle, positionStyle])},[_t("default")])}},
staticRenderFns: [],
  name: 'qx-vbox',
  mixins: [hbox$1],
  beforeCreate: function beforeCreate () {
  ...
  methods: {
    applyChildMargins: function applyChildMargins () {
      ...
      if (this.children.length > 0) {
        ...
        this.children[0].$margin = null;
        for (var i = 1; i < this.children.length; i += 1) {
          this$1.children[i].$margin = [this$1.$spacing, 0, 0, 0];
        }
      }
    }
  }
...
__$styleInject(".qxw.vbox-legacy>.-justify{position:relative;overflow:hidden}.qxw.vbox-legacy.justify-center>.-justify{top:50%;-webkit-transform:translateY(-50%);-ms-transform:translateY(-50%);transform:translateY(-50%)}.qxw.vbox-legacy.justify-end>.-justify{top:100%;-webkit-transform:translateY(-100%);-ms-transform:translateY(-100%);transform:translateY(-100%)}.qxw.vbox-legacy>.-justify>*,.qxw.vbox-legacy>.-justify>.qxw.self-align-start{float:left;clear:both}.qxw.vbox-legacy.align-center>.-justify>*,.qxw.vbox-legacy>.-justify>.qxw.self-align-center{left:50%;-webkit-transform:translateX(-50%);-ms-transform:translateX(-50%);transform:translateX(-50%)}.qxw.vbox-legacy.align-end>.-justify>*,.qxw.vbox-legacy>.-justify>.qxw.self-align-end{margin-left:auto!important;margin-right:0!important}",undefined);

var vBoxLegacy = {
render: function(){with(this){return _h('div',{staticClass:"qxw vbox-legacy",class:classes,style:([boxStyle, paddingStyle, sizeStyle, positionStyle])},[_h('div',{ref:"root",staticClass:"-justify"},[_t("default")])])}},
staticRenderFns: [],
  mixins: [vbox$1, legacyBoxBase],
  beforeCreate: function beforeCreate () {
    this._retainPercentageX = true;
  },
  computed: {
    classes: function classes () {
      var cssClass = hbox$1.computed.classes.call(this);
    ...
    floatingSpace: reactive(function floatingSpace () {
      ...
      if (this.flexCount) {
        ...
        for (var i = 0; i < this.children.length; i += 1) {
          // TODO: exclude non displayed
          if (!this$1.children[i].flex) {
            fixedSpace += this$1.children[i].outerHeight;
          }
          if (i > 0) {
            fixedSpace += this$1.$spacing;
          }
...
__$styleInject(".qxw.flowbox .qxw.box.flow.inline{display:-webkit-inline-box;display:-ms-inline-flexbox;display:inline-flex}",undefined);

var flowbox = {
render: function(){with(this){return _h('div',{staticClass:"qxw flowbox",class:classes,style:([boxStyle, sizeStyle, positionStyle])},[_h('qx-scroller',{attrs:{"height":"100%"}},[_h('div',{staticClass:"valign-container"},[_h('qx-hbox',{ref:"root",staticClass:"inline",class:vAlignClass,attrs:{"flow":"","spacing":$spacing,"justify":$justify,"items-align":$itemsAlign,"padding":$padding,"width":"100%"}},[_t("default")])])])])}},
staticRenderFns: [],
  name: 'qx-flowbox',
  mixins: [rect, boxBase],
  props: {
  ...
  computed: {
    vAlignClass: function vAlignClass () {
...
__$styleInject(".qxw.hsplitter>.qxw.handle{width:12px;height:100%;background-color:#ccc}.qxw.vsplitter>.-justify>.qxw.handle,.qxw.vsplitter>.qxw.handle{width:100%;height:12px;background-color:#ccc}",undefined);

var initialSiblingSize = 0;

var splitterhandle = {
render: function(){with(this){return _h('div',{directives:[{name:"movable",rawName:"v-movable"}],staticClass:"qxw handle",on:{"pointerdown":startResize,"move":resizeSibling}})}},
staticRenderFns: [],
  ...
  methods: {
    startResize: function startResize (evt) {
    ...
    resizeSibling: function resizeSibling (evt) {
      if (this.parent.orientation === 'h') {
        var offsetX = evt.detail.x;
        var width = ((initialSiblingSize + (offsetX * this.offsetMultiplier)) /
          this.parent.getInnerWidth()) * 100;
        this.pane.$width = width + "%";
      } else {
        var offsetY = evt.detail.y;
        var height = ((initialSiblingSize + (offsetY * this.offsetMultiplier)) /
...
var SplitterHandle = vue.extend(splitterhandle);
...
var splitterBase = {
  ...
  mounted: function mounted () {
    ...
    for (var i = children.length - 1; i > 0; i -= 1) {
      this$1.children[i].before(SplitterHandle);
    }
  },
  computed: {
    spacing: function spacing () {
...
__$styleInject(".qxw.overlay{z-index:1}",undefined);

var overlay = {
render: function(){with(this){return (open)?_h('div',{staticClass:"qxw fixed overlay",class:classes,style:([boxStyle, paddingStyle, sizeStyle, positionStyle]),on:{"click":function($event){$event.stopPropagation();},"!pointerdown":function($event){closeOverlay($event)}}},[_t("default")]):_e()}},
staticRenderFns: [],
  name: 'qx-overlay',
  mixins: [rect, overlayContainer],
  // extends: rect,
  ...
  computed: {
    ...
    parentOverlay: function parentOverlay () {
      // locate parent overlay
      var parentOverlay = this.app;
      var parent = this.parent;
  ...
  methods: {
    contains: function contains (el) {
      var contains = this.open && rect.methods.contains.call(this, el);
      if (!contains && this.$overlayPosition) {
        contains = this.parent.$el.contains(el);
      }
      return contains
    },
    showOverlay: function showOverlay (evt) {
      if (this.toggle) {
        this.open = !this.open;
      } else if (!this.open) {
        this.$top = evt.pageY;
    ...
    pointerWatcher: function pointerWatcher (evt) {
  ...
  watch: {
    open: function open (isOpen) {
      if (!isOpen) {
        ...
        this.$nextTick(function repositionOverlay () {
          if (this.open) {
            var menuWidth = this.getOuterWidth();
            var menuHeight = this.getOuterHeight();
            if (this.$overlayPosition === 'bottom') {
    ...
    $showOn: function $showOn (val, oldVal) {
...
__$styleInject(".qxw.overlay.context-menu{background-color:#cbe1e7;min-width:120px}.qxw.overlay.context-menu>*{padding:4px 8px}.qxw.overlay.context-menu>:hover{background-color:#3f7ed1;color:#fff}",undefined);
...
var menubar = {
render: function(){with(this){return _h('div',{staticClass:"qxw menu-bar",class:classes,style:([boxStyle, paddingStyle, sizeStyle, positionStyle])},[_h('qx-hbox',{ref:"root",attrs:{"height":"100%","width":"100%","items-align":"stretch","spacing":$spacing,"justify":$justify}},[_t("default")])])}},
staticRenderFns: [],
  name: 'qx-menu-bar',
  mixins: [rect],
  props: {
  ...
  computed: {
    showMenusOn: function showMenusOn () {
      var oneOpen = this.overlays.some(function (o) { return o.open; });
      return oneOpen ? 'pointerenter' : 'click'
    }
  },
  watch: {
    showMenusOn: function showMenusOn$1 (val) {
      this.overlays.forEach(function (menu) {
        var m = menu;
        m.showOn = val;
      });
    }
...
__$styleInject(".qxw.icon.menu{padding:4px 8px}.qxw.icon.menu.active{background-color:#3f7ed1;color:#fff}",undefined);

function embeddedOverlay (showOn, overlayPosition, autoClose) {
  return {
    data: function data () {
      return {
        showOn: showOn,
...
var menu = {
render: function(){with(this){return _h('div',{staticClass:"qxw icon menu",class:classes,style:([boxStyle, paddingStyle, sizeStyle, positionStyle])},[_h('div',{class:$height ? 'valign-center' : ''},[(src)?_h('img',{style:(imgStyle),attrs:{"src":src}}):_e()," ",($text)?_h('label',[_s($text)]):_e()])," ",_h('qx-contextmenu',{ref:"root",attrs:{"auto-close":autoClose,"show-on":showOn,"overlay-position":overlayPosition}},[_t("default")])])}},
staticRenderFns: [],
  name: 'qx-menu',
  ...
  computed: {
    classes: function classes () {
      var classes = icon.computed.classes.call(this);
      classes.active = this.open;
      return classes
    },
    $imgAlign: function $imgAlign () {
      return 'start'
    }
  }
};

__$styleInject(".qxw.icon.menu.sub{padding-right:16px!important;background:no-repeat right 4px center/8px url(\"data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' width='306' height='306' viewBox='0 0 306 306'%3E%3Cpath d='M94.35 0l-35.7 35.7L175.95 153 58.65 270.3l35.7 35.7 153-153z'/%3E%3C/svg%3E\")}.qxw.icon.menu.sub.active{background-color:#3f7ed1;color:#fff}",undefined);

var submenu = {
  name: 'qx-sub-menu',
  mixins: [menu, embeddedOverlay('pointerenter', 'right', true)],
  class: {
    sub: true
  },
  props: {
  ...
  computed: {
    classes: function classes () {
      var classes = menu.computed.classes.call(this);
      classes.active = this.open;
      return classes
    }
  }
